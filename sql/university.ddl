...
CREATE TABLE `university`.`student` (
  ...
  branch VARCHAR(5),
  program VARCHAR(100),
  foreign key student(program) references program(name),
...
CREATE TABLE `university`.`faculty` (
  ...
  `password` VARCHAR(90) NOT NULL,
  dept_id varchar(45),
  foreign key faculty(dept_id) references department(dept_id),
...
CREATE TABLE has_course(
    ...
    foreign key has_course(program) references program(name),
    foreign key has_course(c_id) references course(c_id),
...
CREATE TABLE final_grade(
    ...
    foreign key final_course(student_id) references student(student_id),
    foreign key final_course(c_id) references course(c_id),
...
CREATE TABLE section_room(
	sec_id INT NOT NULL,
    class_id VARCHAR(45) NOT NULL,
	foreign key section_room(sec_id) references session(sec_id),
    foreign key section_room(class_id) references classroom(class_id),
...
CREATE TABLE manages(
	...
	foreign key manages(faculty_id) references faculty(faculty_id),
    foreign key manages(dept_id) references department(dept_id),
...
CREATE TABLE offer_course (
    ...
    dept_id VARCHAR(45) NOT NULL,
    notes VARCHAR(45),
    foreign key offer_course(c_id) references course(c_id),
    foreign key offer_course(dept_id) references department(dept_id),
...
CREATE TABLE has_program (
    ...
    dept_id VARCHAR(45) NOT NULL,
    notes VARCHAR(45),
    foreign key has_program(program) references program(name),
    foreign key has_program(dept_id) references department(dept_id),
